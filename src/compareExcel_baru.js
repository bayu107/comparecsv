...
if (file1 && file2) {
    ...
    // Fungsi untuk menambahkan hasil perbandingan ke dalam file yang ada
    const appendToFile = (filePath, resultCSV) => {
        if (fs.existsSync(filePath)) {
            // Jika file sudah ada, tambahkan hasil baru ke file yang ada
            fs.appendFileSync(filePath, '\n' + resultCSV.join('\n'));
        } else {
            // Jika file belum ada, buat file baru dan tulis hasil
            fs.writeFileSync(filePath, resultCSV.join('\n'));
        }
    };

    // Fungsi untuk menambahkan header ke file CSV jika file baru
    const addHeaderIfNeeded = (filePath, header) => {
        if (!fs.existsSync(filePath)) {
            // Jika file belum ada, buat file dan tulis header
    ...
    compareColumns.forEach(column => {
        ...
        // Tentukan folder untuk kolom yang akan dibuat jika belum ada
        ...
        // Tentukan path output file untuk kolom ini
        const outputFilePath = path.join(columnFolderPath, 'comparison_result.csv');

        // Menambahkan header jika file baru
        addHeaderIfNeeded(outputFilePath, ["\"Perbandingan Tanggal Modifikasi Jakarta\"", "\"Perbandingan Activity Type\""]);

        // Menambahkan hasil ke file yang ada
        appendToFile(outputFilePath, resultCSV);
